...
public:
    ...
    void redc_req(ScoreBoard_Entry& matrix_sbe, uint8_t _idx, bool _isSigned); //send from zbuffer
    void accu_one_row(std::array<uint8_t, 8> row_data, uint8_t idx); //send from zbuffer
    uint8_t get_dest(uint8_t idx) { return dest[idx]; }
...
private:
    ...
    std::array<int32_t, 8> Redc_RESULT_s{};
    std::array<uint32_t, 8> Redc_RESULT_u{};
    std::vector<uint8_t> sizem;
    std::vector<uint8_t> uimm3;
...
public:
    statistics::Vector ew_op_accepted;
    statistics::Vector redc_executed;
    statistics::Vector acc_executed;
};
} //namespace gem5
