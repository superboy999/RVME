...
public:
    ...
    uint8_t readLock(uint32_t phyreg_idx);
    void readrls(uint32_t phyreg_idx, uint8_t idx);
    uint8_t get_readLock(uint32_t phyreg_idx);
    bool check_readLock(uint32_t phyreg_idx, uint8_t idx);
...
private:
    ...
    std::vector<uint32_t> Read_tail;
    std::vector<uint32_t> Read_head;
    std::vector<std::vector<uint32_t>> Read_commit;
    std::vector<std::vector<uint32_t>> Read_valid;
