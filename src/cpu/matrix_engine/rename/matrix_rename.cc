...
#include "debug/MatrixRenameReadLock.hh"
    ...
    accPRFs(params.accPRFs)
{
    ...
    Read_head.resize(numPhysicalRegs, 1);
    Read_tail.resize(numPhysicalRegs, 1);
    Read_commit.resize(numPhysicalRegs);
    Read_valid.resize(numPhysicalRegs);
    for(uint32_t i = 0; i < numPhysicalRegs; i++) {
        Read_commit[i].resize(64, 0);
        Read_valid[i].resize(64, 0);
        Read_valid[i][0] = 1;
    }
    // init RAT[0-3] for ACC reg
...
void MatrixRename::set_matrixEnginePtr(MatrixEngine* _matrix_engine) 
{
...
uint8_t MatrixRename::readLock(uint32_t phyreg_idx)
{
    uint8_t return_idx = Read_tail[phyreg_idx];
    Read_valid[phyreg_idx][Read_tail[phyreg_idx]] = 0;
    if(Read_tail[phyreg_idx] == 63)
        Read_tail[phyreg_idx] = 0;
    else
        Read_tail[phyreg_idx]++;
    DPRINTF(MatrixRenameReadLock, "RenamereadLock PR:%d\n", phyreg_idx);
    return return_idx;
}

void MatrixRename::readrls(uint32_t phyreg_idx, uint8_t idx)
{
    Read_commit[phyreg_idx][idx] = 1;
    while(Read_commit[phyreg_idx][Read_head[phyreg_idx]]) {
        Read_valid[phyreg_idx][Read_head[phyreg_idx]] = 1;
        if(Read_head[phyreg_idx] == 63)
            Read_head[phyreg_idx] = 0;
        else
            Read_head[phyreg_idx] ++;
        if(Read_head[phyreg_idx] == Read_tail[phyreg_idx])
            break;
    }
    DPRINTF(MatrixRenameReadLock, "RenamereadRls PR:%d\n", phyreg_idx);
}

uint8_t MatrixRename::get_readLock(uint32_t phyreg_idx)
{
    return Read_tail[phyreg_idx];
}

bool MatrixRename::check_readLock(uint32_t phyreg_idx, uint8_t idx)
{
    return (idx == 0) ? Read_valid[phyreg_idx][63] : Read_valid[phyreg_idx][idx - 1];
}

void MatrixRename::set_freeReg(uint32_t phyreg_idx)
{
    if(phyreg_idx < tilePRFs){
        // this->matrix_engine->matrix_reg->set_reg_zero(phyreg_idx);
        TileFreeList_write++;
