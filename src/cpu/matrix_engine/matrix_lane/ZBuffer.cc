...
void ZBuffer::set_matrixrename(MatrixRename* _matrix_rename)
{
    matrix_rename = _matrix_rename;
}
...
void ZBuffer::set_md(uint8_t _md, uint32_t _src3_memdep_idx, uint8_t _dstReg_idx)
{
    if(src_set == 0) {
        src_set = 1;
        src3_memdep_idx[send_index] = _src3_memdep_idx;
        md[send_index] = _md;
        dstReg_idx[send_index] = _dstReg_idx;
    }
    else {
        src_set++;
        src3_memdep_idx[send_index ^ 1] = _src3_memdep_idx;
        md[send_index ^ 1] = _md;
        dstReg_idx[send_index ^ 1] = _dstReg_idx;
    }
    DPRINTF(ZBuffer, "zbuffer set dst_idx = %d, old_dst = %d, send_index = %d, md[0] = %d, md[1] = %d, src[0] = %d, scr[1] = %d, src_set = %d, \n", dstReg_idx[send_index], dstReg_idx[send_index^1], send_index, md[0], md[1], src3_memdep_idx[0], src3_memdep_idx[1], src_set);
}

void ZBuffer::set_dst(uint8_t _dstReg_idx)
{
    busy = true;
    uint8_t old_dst = dstReg_idx[send_index ^ 1];
    if(src_set == 1)
        dstReg_idx[send_index] = _dstReg_idx;
    else
        dstReg_idx[send_index ^ 1] = _dstReg_idx;
    if(src_set != 1)
        old_dst = dstReg_idx[send_index];
    DPRINTF(ZBuffer, "zbuffer set dst_idx = %d, old_dst = %d, send_index = %d, md[0] = %d, md[1] = %d, src[0] = %d, scr[1] = %d, src_set = %d, \n", dstReg_idx[0], old_dst, send_index, md[0], md[1], src3_memdep_idx[0], src3_memdep_idx[1], src_set);
}
...
void ZBuffer::send2EWU_req() //这个应该是在zbuffer eval里面调用的
{
   ...
   if(matrix_engine->ew_unit->isIdle() == true){
    ...
    busy = true;
    matrix_engine->ew_unit->acc_req(dstReg_idx[send_index], row_size, matrix_lane->lane_idx, isSigned);
}
...
void ZBuffer::evaluate()
{
    ...
    if(send2EWUReq){
        ...
        if(send_cnt == row_size){
            //send finish
            DPRINTF(ZBuffer, "send_index = %d, dstReg_idx[send_index] = %d, md[send_index] = %d, src3_memdep_idx[send_index] = %d\n", 1 ^ send_index, dstReg_idx[1 ^ send_index], md[1 ^ send_index], src3_memdep_idx[1 ^ send_index]);
            send2EWUReq = false;
            send_cnt = 0;
            DPRINTF(ZBuffer, "Zbuffer send2EWQReq done! \n");
    ...
    // }
    if(data_ready&&(matrix_engine->matrix_rename->get_PR_vld(dstReg_idx[send_index]) || send_cnt != 0) && matrix_engine->matrix_rob->macc_raw_solved(md[send_index], src3_memdep_idx[send_index])){
    // if(data_ready){
        // send_req();
        if(matrix_engine->ew_unit->isOccupied(matrix_lane->lane_idx) == false && matrix_engine->matrix_reg->wtport_valid(dstReg_idx[send_index], 0, send_cnt) && matrix_engine->matrix_reg->rdport_valid(dstReg_idx[send_index], 0, send_cnt)){
            matrix_engine->matrix_rename->set_PR_vld(dstReg_idx[send_index], false);
            DPRINTF(ZBuffer, "ZBuffer set phy vld: send_index = %d, dstReg_idx[0] = %d, dstReg_idx[1] = %d, md[0] = %d, md[1] = %d, src3_memdep_idx[0] = %d, src3_memdep_idx[1] = %d\n", send_index, dstReg_idx[0], dstReg_idx[1], md[0], md[1], src3_memdep_idx[0], src3_memdep_idx[1]);
            matrix_engine->matrix_reg->occupy_wtport(dstReg_idx[send_index], 0, send_cnt);
            matrix_engine->matrix_reg->occupy_rdport(dstReg_idx[send_index], 0, send_cnt);
            send2EWU_req();
            src_set--;
            send_index = send_index ^ 1;
        }
        // else if(signed_writeRequest[0] || unsigned_writeRequest[0]) {
        //     wait_ew = 1;
        // }
    }
