...
void MatrixLane::set_matrixrename(MatrixRename* _matrix_rename)
{
    matrix_rename = _matrix_rename;
    zbuffer->set_matrixrename(matrix_rename);
}
...
void MatrixLane::issue_inst(ScoreBoard_Entry* matrix_sbe)
{
    ...
    busy = true;
    md = matrix_sbe->md;
    src3_memdep_idx = matrix_sbe->src3_memdep_idx;
    dstReg_idx = lane_minst->zreg_idx;
    startTicking();
    InstNum++;
    issue_req = true;
    DPRINTF(Matrix_Lane, "Matrix Lane issue inst src3_memdep_idx = %d\n", src3_memdep_idx);
    //startTicking all that stuff!
}
...
void MatrixLane::evaluate()
{
    ...
    } else if((!saveZDone&&sendtoZ&&!start_transZ)||(start_transZ)){
        
        // computeDone = false;
        if(!start_transZ){
            sendtoZ = false;
            busy = false;
            zbuffer->set_md(md, src3_memdep_idx, dstReg_idx);
            DPRINTF(Matrix_Lane, "Set md\n");
        }
        start_transZ = true;
        if(toZcnt < tranz_minst.matrix_size.num_row){
            for (uint32_t i = 0; i < tranz_minst.matrix_size.num_column; i++){
    ...
    if(!computeDone){
        ...
        if(compute_units[0]->computeDone()){
            computeDone = true;
            // zbuffer->set_dst(lane_minst->zreg_idx);
            // computing = false;
...
void MatrixLane::regStats(){
    ...
    BusyCycles
        ...
        .desc("Count how many cycles this matrix lane has computed");
    LaneActiveCycles
        .name(name() + ".LaneActiveNumber")
        .desc("Count how many cycles this matrix lane is active");
}
