...
public:
    ...
    void set_md(uint8_t _md, uint32_t _src3_memdep_idx, uint8_t _dstReg_idx);
    void set_dst(uint8_t _dstReg_idx);

    void set_matrixrename(MatrixRename* _matrix_rename);
...
private:
    ...
    bool sendReq;
    bool send2EWUReq;
    uint32_t src3_memdep_idx[2];
    uint8_t md[2];
    uint8_t send_index;
    uint8_t src_set;
    uint8_t wait_ew;
    uint8_t send_cnt;
    uint8_t dstReg_idx[2];
};
