...
void ReorderBuffer::evaluate()
{
    ...
    //     if(matrix_engine->matrix_rename->checkLock(free_reg[i])) {
    //         matrix_engine->matrix_rename->set_freeReg(free_reg[i]);
    //         free_reg.erase(free_reg.begin() + i);
    //         i--;
    //     }
    // }
    ...
    if((rob[head]->executed) && (rob[head]->valid))
    {
        ...
        // else if(rob[head]->old_dst_vld && rob[head]->old_dst != 99) {
        //     free_reg.push_back(rob[head]->old_dst);
        // }
    ...
    for(uint8_t i = 0; i < 8; i ++)
    {
        ...
        if(memdep_table[i][mdhead[i]].valid)
        {
            ...
            memdep_table[i][mdhead[i]].valid = false;
            if(mdhead[i] == MDU_depth-1){
                mdhead[i] = 0;
            } else{
                mdhead[i] ++;
            }
            md_valid[i][mdhead[i]] = 1;
            mdvalid_element[i] --;
        }
    }
}
...
{
    DPRINTF(ReorderBuffer, "set_md_entry: idx = %d, mdvalid_element = %d\n", idx, mdvalid_element[idx]);
    ...
    md_valid[idx][mdtail[idx]] = 0;

    mdvalid_element[idx] ++;
    DPRINTF(ReorderBuffer, "after set_md_entry: idx = %d, mdvalid_element = %d, md_valid[%d][%d] = %d\n", idx, mdvalid_element[idx], idx, mdtail[idx], md_valid[idx][mdtail[idx]]);
    return return_tail;
}

void ReorderBuffer::set_md_entry_valid(uint8_t idx, uint32_t entry_idx)
{
    DPRINTF(ReorderBuffer, "set_md_entry_valid, idx = %d, entry_idx = %d\n", idx, entry_idx);
...
{
    DPRINTF(ReorderBuffer, "set_rob_entry, valid_element = %d\n", valid_element);
    ...
    valid_element ++;
    ROB_write++;
    DPRINTF(ReorderBuffer, "set_rob_entry, return tail = %d\n", return_tail);
    return return_tail;
}

void ReorderBuffer::set_rob_entry_executed(uint32_t idx)
{
    DPRINTF(ReorderBuffer, "set_rob_entry_executed, rob[%d]->executed = %d\n", idx, rob[idx]->executed);
