...
public:
    ...
    // bool macc_raw_solved(uint8_t idx, uint32_t check_tail) {return mdhead[idx] + 1 == check_tail || (mdhead[idx] == MDU_depth - 1 && check_tail == 0);}
    bool raw_solved(uint8_t idx, uint32_t check_tail) {return md_valid[idx][check_tail];}
    bool macc_raw_solved(uint8_t idx, uint32_t check_tail) {return (check_tail == 0) ? md_valid[idx][MDU_depth-1] : md_valid[idx][check_tail-1];}
...
private:
    ...
    std::vector<std::vector<int>> md_valid;
    std::vector<int> free_reg;
public:
