...
static alignas(CACHELINE_SIZE) acc_t g_zero_tile[DIM_I * DIM_J] = {0};
...
static void sp_tiled_norm(const acc_t* in, acc_t* out,
  ...
  if (act == SOFTMAX) {
    for (size_t i = 0; i < I; i++) {
      ...
      printf("[DEBUG][SOFTMAX] sum_exp in acc3 before col-reduce add:\n");
...
static void tiled_norm_auto(const size_t dim_I, const size_t dim_J,
  ...
  while (true) {
    ...
    if (is_spad_rows_satisfied(tile_I, tile_J + 1, 0) &&
    ...
    if (is_spad_rows_satisfied(tile_I + 1, tile_J, 0) &&
