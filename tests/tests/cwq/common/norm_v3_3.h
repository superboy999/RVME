#ifndef __NORM_V3_H__
#define __NORM_V3_H__
...
long long norm_v3_cnt = 0;

// void add_norm_v3_cnt(long long x) {
//   norm_v3_cnt = norm_v3_cnt + x;
// }

static alignas(CACHELINE_SIZE) acc_t g_zero_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 0};
static alignas(CACHELINE_SIZE) acc_t qa_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 1};
static alignas(CACHELINE_SIZE) acc_t qb_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 1};
static alignas(CACHELINE_SIZE) acc_t qc_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 1};
static alignas(CACHELINE_SIZE) acc_t qs_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 1};
static alignas(CACHELINE_SIZE) acc_t shift_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 1};
static alignas(CACHELINE_SIZE) acc_t half_shift_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 1};
static alignas(CACHELINE_SIZE) acc_t lshift_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 9};
static alignas(CACHELINE_SIZE) acc_t rshift_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 22};
static alignas(CACHELINE_SIZE) acc_t shiftclip_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 31};
static alignas(CACHELINE_SIZE) acc_t dim_J_log2_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 0};
static alignas(CACHELINE_SIZE) acc_t const1_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 1};
static alignas(CACHELINE_SIZE) acc_t const2_tile[DIM_I * DIM_J] = {[0 ... DIM_I*DIM_J-1] = 1};
...
static void sp_tiled_norm(const acc_t* in, acc_t* out,
  ...
  // const uint64_t C_temp_sp_addr_start = C_sp_addr_start + 32 * 32; // 临时存放区，防止覆盖
  ...
  if(act == SOFTMAX){
    for (size_t i0 = 0; i0 < I * DIM_I; i0++) {
      for (size_t j = 0; j < J; j += (size_t)C_blocks) {
        ...
        // printf("[DEBUG][NORM] input_sp_addr = 0x%lx = %llu + (%llu * %llu + %llu) * %llu\n", input_sp_addr, C_sp_addr_start, i0, (J * DIM_J), j * DIM_J, sizeof(acc_t));
        size_t bytes = cols * DIM_J * sizeof(acc_t);
        // printf("[DEBUG][NORM] Move-in input: Input[%zu, %zu]-[%zu, %zu] (%zu bytes) from Mem addr 0x%lx-0x%lx to SPM addr 0x%lx-0x%lx\n",
  ...
  } else if (act == LAYERNORM || act == IGELU){
  ...
  msync_spm();
  assert(act == SOFTMAX || act == LAYERNORM || act == IGELU); /* 仅支持ReLU、Softmax */
  assert(out != NULL);
  if (act == SOFTMAX) {
    for (size_t i = 0; i < I; i++) {
      msettilemi(DIM_I);
      // pass 1: get max_q
      // printf("[DEBUG][SOFTMAX] Get max\n");
      ...
      // printf("[DEBUG][SOFTMAX] Column reduce max\n");
      // mredcmax_w(TILE_NUM+3, TILE_NUM+3); /* acc3 col-reduce max */
      mredcmax_w_i(TILE_NUM+3, TILE_NUM+3, 0);
      // add_norm_v3_cnt(8 * 7);
      // printf("[DEBUG][SOFTMAX] q = q - max\n");
      ...
      msub_w_mm(TILE_NUM+3, TILE_NUM+3, TILE_NUM+0); /* -max = 0 - max */
      // add_norm_v3_cnt(8 * 8);
      for (size_t j = 0; j < J; j++) {
        uint64_t input_sp_addr = C_sp_addr_start
          + (uint64_t)(((i * DIM_I) * (J * DIM_J) + j * DIM_J) * sizeof(acc_t));
        ...
        madd_w_mv_i(TILE_NUM+2, TILE_NUM+2, TILE_NUM+3, 0); /* q + (-max) */
        // add_norm_v3_cnt(8 * 8);
        // msub_w_mv_i(TILE_NUM+2, TILE_NUM+3, TILE_NUM+2, 0); /* q = q - max */
      ...
      // printf("[DEBUG][SOFTMAX] Calculate exp and sum\n");
      for (size_t j = 0; j < J; j++) {
        uint64_t input_sp_addr = C_sp_addr_start
          + (uint64_t)(((i * DIM_I) * (J * DIM_J) + j * DIM_J) * sizeof(acc_t));
        msettileni(DIM_J);
        ...
        mlcte32_spm(TILE_NUM+2, input_sp_addr, (int)(J * DIM_J * sizeof(acc_t)), 0);
        /* 加载-qln2_inv矩阵 */
        // printf("[DEBUG][SOFTMAX] Load const2_tile\n");
        msync_spm();
        ...
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0);
        // printf("[DEBUG][SOFTMAX] z = q * -qln2_inv\n");
        mmul_w_mm(TILE_NUM+1, TILE_NUM+2, TILE_NUM+0); /* z = q * -qln2_inv */
        // add_norm_v3_cnt(8 * 8);
        /* 加载shift矩阵 */
        // printf("[DEBUG][SOFTMAX] Load shift_tile\n");
        ...
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0);
        // printf("[DEBUG][SOFTMAX] z >> shift\n");
        msrl_w_mm(TILE_NUM+1, TILE_NUM+1, TILE_NUM+0); /* z = z >> 16 */
        // add_norm_v3_cnt(8 * 8);
        msync_spm();
        dmaload_spm((uint64_t)shiftclip_tile, C_temp_sp_addr_start+0, 1);
        dmaload_spm((uint64_t)shiftclip_tile+64, C_temp_sp_addr_start+64, 1);
        dmaload_spm((uint64_t)shiftclip_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        dmaload_spm((uint64_t)shiftclip_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0);
        mmin_w_mm(TILE_NUM+1, TILE_NUM+1, TILE_NUM+0); /* z = min(z, shiftclip) */
        // add_norm_v3_cnt(8 * 8);
        /* q = q * qs */
        // printf("[DEBUG][SOFTMAX] Load qs_tile\n");
        ...
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0);
        // printf("[DEBUG][SOFTMAX] q = q * qs\n");
        mmul_w_mm(TILE_NUM+2, TILE_NUM+2, TILE_NUM+0); /* q = q * qs */
        // add_norm_v3_cnt(8 * 8);
        /* 加载qln2矩阵 */
        // printf("[DEBUG][SOFTMAX] Load const1_tile\n");
        ...
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0);
        // printf("[DEBUG][SOFTMAX] qp = qp + b\n");
        madd_w_mm(TILE_NUM+2, TILE_NUM+2, TILE_NUM+0); /* qp = qp + b */
        // add_norm_v3_cnt(8 * 8);
        /* 加载half_shift矩阵 */
        // printf("[DEBUG][SOFTMAX] Load half_shift_tile\n");
        ...
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0);
        // printf("[DEBUG][SOFTMAX] q_exp = q_exp + qc\n");
        madd_w_mm(TILE_NUM+2, TILE_NUM+2, TILE_NUM+0); /* q_exp = q_exp + qc */
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG][SOFTMAX] q_exp >> z\n");
      ...
      mredcadd_w_i(TILE_NUM+3, TILE_NUM+3, 0);
      // add_norm_v3_cnt(8 * 7);
      ...
      msync_spm();
      mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0);
      msrl_w_mm(TILE_NUM+3, TILE_NUM+3, TILE_NUM+0);
      // add_norm_v3_cnt(8 * 8);
      // printf("[DEBUG][SOFTMAX] sum_exp = 1/sum_exp\n");
      mlut_w_i(TILE_NUM+3, TILE_NUM+3, 0, 0); /* sum_exp -> 1/sum_exp */
      // pass 3: normalize
      // printf("[DEBUG][SOFTMAX] Normalize\n");
      for (size_t j = 0; j < J; j++) {
        uint64_t input_sp_addr = C_sp_addr_start
          + (uint64_t)(((i * DIM_I) * (J * DIM_J) + j * DIM_J) * sizeof(acc_t));
        msettileni(DIM_J);
        ...
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0);
        // printf("[DEBUG][SOFTMAX] q >> (shift/2)\n");
        msrl_w_mm(TILE_NUM+2, TILE_NUM+2, TILE_NUM+0); /* q >> (shift/2) */
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG][SOFTMAX] q * (1/sum_exp)\n");
  ...
  /* ------------------------------------------ ed2 ------------------------------- */
    for( size_t i = 0; i < I; i++) {
      ...
      for (size_t j = 0; j < J; j++) {
        ...
        msub_w_mm(TILE_NUM+1, TILE_NUM+3, TILE_NUM+1); /* -mean = 0 - mean */
        madd_w_mv_i(TILE_NUM+2, TILE_NUM+2, TILE_NUM+1, 0); /* q = (q - mean) -> acc2 Q24.8 */
        // add_norm_v3_cnt(8 * 8);
      ...
      msub_w_mm(TILE_NUM+3, TILE_NUM+3, TILE_NUM+0); /* A: {0/negative} -> acc3 */
      // add_norm_v3_cnt(8 * 8);
      ...
      msub_w_mm(TILE_NUM+1, TILE_NUM+3, TILE_NUM+0); /* C = 1 - B -> acc1 Q16.16 */ 
      // add_norm_v3_cnt(8 * 8);
      ...
      // add_norm_v3_cnt(8 * 32);
      // printf("[DEBUG] calculate diff and frac\n");
      msub_w_mm(TILE_NUM+0, TILE_NUM+2, TILE_NUM+0); /* diff = 1/sqrt(idx + 1) - 1/sqrt(idx) Q8.24*/
      // add_norm_v3_cnt(8 * 8);
      ...
      // add_norm_v3_cnt(8 * 8);
      msub_w_mm(TILE_NUM+1, TILE_NUM+2, TILE_NUM+1); /* (1.5 - 0.5 * y0^2 * x) -> acc1 Q16.16 */
      // add_norm_v3_cnt(8 * 8);
      ...
      // add_norm_v3_cnt(8 * 8);
      msub_w_mm(TILE_NUM+1, TILE_NUM+2, TILE_NUM+1); /* (1.5 - 0.5 * y1^2 * x) -> acc1 Q16.16 */
      // add_norm_v3_cnt(8 * 8);
    ...
    for (size_t i = 0; i < I; i++) {
      for (size_t j = 0; j < J; j++) {
        ...
        uint64_t input_sp_addr = C_sp_addr_start
          + (uint64_t)(((i * DIM_I) * (J * DIM_J) + j * DIM_J) * sizeof(acc_t));
        mlcte32_spm(TILE_NUM+1, input_sp_addr, (int)(J * DIM_J * sizeof(acc_t)), 0); /* C tile -> acc1 */
        msync_spm();
        dmaload_spm((uint64_t)const3_tile, C_temp_sp_addr_start, 1);
        dmaload_spm((uint64_t)const3_tile+64, C_temp_sp_addr_start+64, 1);
        dmaload_spm((uint64_t)const3_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        dmaload_spm((uint64_t)const3_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* 31 -> acc0 */
        msra_w_mm(TILE_NUM+2, TILE_NUM+1, TILE_NUM+0); /* x >> 31  -> acc2 Q32.0 */
        // add_norm_v3_cnt(8 * 8);
        msync_spm();
        // dmaload_spm((uint64_t)const4_tile, C_temp_sp_addr_start, cachelines);
        dmaload_spm((uint64_t)const4_tile, C_temp_sp_addr_start, 1);
        dmaload_spm((uint64_t)const4_tile+64, C_temp_sp_addr_start+64, 1);
        dmaload_spm((uint64_t)const4_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        dmaload_spm((uint64_t)const4_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* 2 -> acc0 */
        mmul_w_mm(TILE_NUM+3, TILE_NUM+2, TILE_NUM+0); /* (x >> 31) * 2  -> acc3 Q32.0 */
        // add_norm_v3_cnt(8 * 8);
        msync_spm();
        // dmaload_spm((uint64_t)const5_tile, C_temp_sp_addr_start, cachelines);
        dmaload_spm((uint64_t)const5_tile, C_temp_sp_addr_start, 1);
        dmaload_spm((uint64_t)const5_tile+64, C_temp_sp_addr_start+64, 1);
        dmaload_spm((uint64_t)const5_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        dmaload_spm((uint64_t)const5_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /*  1 -> acc0 */
        madd_w_mm(TILE_NUM+2, TILE_NUM+3, TILE_NUM+0); /* sign x : ((x >> 31) * 2 + 1)  -> acc2 Q32.0 */
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG] abs x -> acc3\n");        
        mmul_w_mm(TILE_NUM+3, TILE_NUM+1, TILE_NUM+2); /* abs x : x * sign x  -> acc3 Q32.0 */
        msync_spm();
        dmaload_spm((uint64_t)const6_tile, C_temp_sp_addr_start, 1);
        dmaload_spm((uint64_t)const6_tile+64, C_temp_sp_addr_start+64, 1);
        dmaload_spm((uint64_t)const6_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        dmaload_spm((uint64_t)const6_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* 2^16 -> acc0 */
        mmul_w_mm(TILE_NUM+3, TILE_NUM+3, TILE_NUM+0); /* abs x * 2^16  -> acc3 Q16.16 */
        msync_spm();
        // dmaload_spm((uint64_t)qb_tile, C_temp_sp_addr_start, cachelines);
        dmaload_spm((uint64_t)qb_tile, C_temp_sp_addr_start, 1);
        dmaload_spm((uint64_t)qb_tile+64, C_temp_sp_addr_start+64, 1);
        dmaload_spm((uint64_t)qb_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        dmaload_spm((uint64_t)qb_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* -qb -> acc0 Q16.16*/
        // printf("[DEBUG] clip\n");
        mmin_w_mm(TILE_NUM+3, TILE_NUM+3, TILE_NUM+0); /* clip(abs(x)) : min(abs x, -qb)  -> acc3 Q16.16 */
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG] calculate clip(abs(x)) + qb:\n");
        msub_w_mm(TILE_NUM+3, TILE_NUM+3, TILE_NUM+0); /* clip(abs(x)) + qb  -> acc3 Q16.16 */
        // add_norm_v3_cnt(8 * 8);
        msync_spm();
        dmaload_spm((uint64_t)const2_tile, C_temp_sp_addr_start, 1);
        dmaload_spm((uint64_t)const2_tile+64, C_temp_sp_addr_start+64, 1);
        dmaload_spm((uint64_t)const2_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        dmaload_spm((uint64_t)const2_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* 8 -> acc0 */
        msra_w_mm(TILE_NUM+3, TILE_NUM+3, TILE_NUM+0); /* clip(abs(x) + qb)  -> acc3 Q24.8 */
        // printf("[DEBUG] calculate (clip(abs(x) + qb))^2:\n");
        mmul_w_mm(TILE_NUM+0, TILE_NUM+3, TILE_NUM+3); /* (clip(abs(x) + qb))^2  -> acc0 Q16.16 */
        // add_norm_v3_cnt(8 * 8);
        msync_spm();
        // dmaload_spm((uint64_t)qc_tile, C_temp_sp_addr_start, cachelines);
        dmaload_spm((uint64_t)qc_tile, C_temp_sp_addr_start, 1);
        dmaload_spm((uint64_t)qc_tile+64, C_temp_sp_addr_start+64, 1);
        dmaload_spm((uint64_t)qc_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        dmaload_spm((uint64_t)qc_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        // printf("[DEBUG] load qc\n");
        mlce32_spm(TILE_NUM+3, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* qc -> acc3 Q16.16 */
        // printf("[DEBUG] calculate q_poly: (clip(abs(x) + qb))^2 + qc:\n");
        madd_w_mm(TILE_NUM+0, TILE_NUM+0, TILE_NUM+3); /* (clip(abs(x) + qb))^2 + qc  -> acc0 Q16.16 */
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG] calculate erf:\n");
        mmul_w_mm(TILE_NUM+0, TILE_NUM+0, TILE_NUM+2); /* erf : ((clip(abs(x) + qb))^2 + qc) * sign x  -> acc0 Q16.16 */
        // add_norm_v3_cnt(8 * 8);
        msync_spm();
        // dmaload_spm((uint64_t)qa_tile, C_temp_sp_addr_start, cachelines);
        dmaload_spm((uint64_t)qa_tile, C_temp_sp_addr_start, 1);
        dmaload_spm((uint64_t)qa_tile+64, C_temp_sp_addr_start+64, 1);
        dmaload_spm((uint64_t)qa_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        dmaload_spm((uint64_t)qa_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        // printf("[DEBUG] load q1\n");
        mlce32_spm(TILE_NUM+2, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* q1 -> acc2 Q16.16 */
        madd_w_mm(TILE_NUM+0, TILE_NUM+0, TILE_NUM+2); /* erf + q1  -> acc0 Q16.16 */ 
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG] calculate IGELU:\n");
        mmul_w_mm(TILE_NUM+0, TILE_NUM+0, TILE_NUM+1); /* GELU(x) = x * (erf + q1)  -> acc0 Q16.16 */
        // add_norm_v3_cnt(8 * 8);
        // msync_spm();
        // dmaload_spm((uint64_t)const1_tile, C_temp_sp_addr_start, 1);
        // dmaload_spm((uint64_t)const1_tile+64, C_temp_sp_addr_start+64, 1);
        // dmaload_spm((uint64_t)const1_tile+64*2, C_temp_sp_addr_start+64*2, 1);
        // dmaload_spm((uint64_t)const1_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        // msync_spm();
        // mlce32_spm(TILE_NUM+2, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* 16 -> acc2 */
        // msra_w_mm(TILE_NUM+0, TILE_NUM+0, TILE_NUM+2); /* IGELU >> 16 -> acc0 Q32.0*/
        mscte32_spm(TILE_NUM+0, input_sp_addr, (int)(J * DIM_J * sizeof(acc_t)), 0); /* acc0 -> C tile (写回 SPM) */
  ...
  // printf("[DEBUG][NORM] Move-out Output\n");
  if (act == SOFTMAX){
  for (size_t i = 0; i < I; i++) {
    for (size_t j = 0; j < J; j += (size_t)C_blocks) {
      size_t cols = min_size(J - j, (size_t)C_blocks);
      // printf("[DEBUG][NORM] cols = min(%zu, %zu) = %zu\n", J - j, (size_t)C_blocks, cols);
      for (size_t i0 = 0; i0 < DIM_I; i0++) {
        ...
        size_t bytes = cols * DIM_J * sizeof(acc_t);
        // printf("[DEBUG][NORM] Move-out C[%zu, %zu]-[%zu, %zu] (%zu bytes) from SPM addr 0x%lx-0x%lx to Mem addr 0x%lx-0x%lx\n",
...
static void tiled_norm(size_t dim_I, size_t dim_J, const acc_t* in, acc_t* out,
  ...
  if (act == SOFTMAX) {
    ...
    // printf("[INFO][NORM] Softmax params: qb=%d, qc=%d, qln2=%d, qsln2_inv=%d, qs=%d, shift=%d, half_shift=%d\n",
        // (int)qb, (int)qc, (int)qln2, (int)qsln2_inv, (int)qs, (int)intermediate_q_bits, (int)half_intermediate_q_bits);
  } else if (act == LAYERNORM) {
    ...
    const float intermediate_scale = 1 << intermediate_q_bits;
    const float S = input_scale_factor;
    ...
    const float const1 = 16.0;
    const float const2 = 8.0;
    const float const3 = 31.0;
    const float const4 = 2.0;
    const float const5 = 1.0;
    const float const6 = 65536.0; //2^16

    acc_t qq1 = (acc_t)(q1 * (1 << 16)); // q1 Q16.16
    acc_t qb_neg_acc = (acc_t)(qb *(-1) * (1 << 16)); // -qb Q16.16
    acc_t qc_acc = (acc_t)(qc * (1 << 16)); // qc Q16.16
    acc_t qconst1 = (acc_t)(const1 * intermediate_scale);
    acc_t qconst2 = (acc_t)(const2 * intermediate_scale);
    acc_t qconst3 = (acc_t)(const3 * intermediate_scale);
    acc_t qconst4 = (acc_t)(const4 * intermediate_scale);
    acc_t qconst5 = (acc_t)(const5 * intermediate_scale);
    acc_t qconst6 = (acc_t)(const6 * intermediate_scale);

    UPDATE_ARRAY(qa_tile, DIM_I * DIM_J, qq1);
    UPDATE_ARRAY(qb_tile, DIM_I * DIM_J, qb_neg_acc);
    UPDATE_ARRAY(qc_tile, DIM_I * DIM_J, qc_acc);
    UPDATE_ARRAY(const1_tile, DIM_I * DIM_J, qconst1);
    UPDATE_ARRAY(const2_tile, DIM_I * DIM_J, qconst2);
    UPDATE_ARRAY(const3_tile, DIM_I * DIM_J, qconst3);
    UPDATE_ARRAY(const4_tile, DIM_I * DIM_J, qconst4);
    UPDATE_ARRAY(const5_tile, DIM_I * DIM_J, qconst5);
    UPDATE_ARRAY(const6_tile, DIM_I * DIM_J, qconst6);
  }
  ...
  for (size_t i0 = 0; i0 < I0; i0++) {
    for (size_t j0 = 0; j0 < J0; j0++) {
      ...
      acc_t* tile_out = NULL;
      // printf("[DEBUG][NORM] j0 = %zu, J0 - 1 = %zu\n", j0, J0 - 1);
      if (j0 == J0 - 1) {
        tile_out = (acc_t*)out + (i0 * tile_I * DIM_I) * stride_out + j0 * tile_J * DIM_J;
      }
      // printf("[INFO][NORM] Outer tile block (%zu, %zu): I=%zu, J=%zu, pre=%p, out=%p\n",
      //     i0, j0, I, J, tile_in, tile_out);
      if ((act == LAYERNORM || act == SOFTMAX || act == IGELU)) {
...
static void tiled_norm_auto(const size_t dim_I, const size_t dim_J,
  ...
  // printf("[INFO][NORM] max_tile_ij_in_bank_c = %d, tile_I = %d, tile_J = %d\n", 
    // (int)max_tile_ij_in_bank_c, (int)tile_I, (int)tile_J);
  size_t temp_size = 0;
  if (act == SOFTMAX || act == IGELU) {
  ...
  while (true) {
    ...
    if (is_spad_rows_satisfied_2d(tile_I, tile_J + 1, temp_size) &&
        (tile_J + 1) * DIM_J <= dim_J_padded) {
      tile_J++;
      increased = true;
      // printf("[DEBUG][NORM] Increase J to %d\n", (int)tile_J);
      continue;
    }
    if (is_spad_rows_satisfied_2d(tile_I + 1, tile_J, temp_size) &&
        (tile_I + 1) * DIM_I <= dim_I_padded) {
      tile_I++;
      increased = true;
      // printf("[DEBUG][NORM] Increase I to %d\n", (int)tile_I);
      continue;
    }
    if (!increased) break;
  }
  // printf("[INFO][NORM] Auto tiling selected: I=%d, J=%d, act=%s\n",
      // (int)tile_I, (int)tile_J,
      // act == LAYERNORM ? "LAYERNORM" : (act == SOFTMAX ? "SOFTMAX" : (act == IGELU ? "IGELU" : "NONE")));
#if PRINT_TILE
  {
    const int bank_c_rows = (tile_I * DIM_I * tile_J * DIM_J) * sizeof(acc_t) / BANK_WIDTH;
    printf("[INFO][NORM] tile_I: %d\n", (int)tile_I);
    printf("[INFO][NORM] tile_J: %d\n", (int)tile_J);
    printf("[INFO][NORM] bankC_used_rows: %d\n", bank_c_rows);
    printf("[INFO][NORM] Bank C utilization: %f%%\n\n", (float)(bank_c_rows * 100) / BANK_C_ROWS);
  }
  // exit(1);
#endif
