  ...
  /* ------------------------------------------ ed2 ------------------------------- */
    ...
    for (size_t i = 0; i < I; i++) {
      for (size_t j = 0; j < J; j++) {
        ...
        dmaload_spm((uint64_t)qb_tile+64*3, C_temp_sp_addr_start+64*3, 1);
        msync_spm();
        mlce32_spm(TILE_NUM+0, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* -qb -> acc0 Q32.0*/
        // printf("[DEBUG] clip\n");
        mmin_w_mm(TILE_NUM+3, TILE_NUM+3, TILE_NUM+0); /* clip(abs(x)) : min(abs x, -qb)  -> acc3 Q32.0 */
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG] calculate clip(abs(x)) + qb:\n");
        msub_w_mm(TILE_NUM+3, TILE_NUM+3, TILE_NUM+0); /* clip(abs(x)) + qb  -> acc3 Q32.0 */
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG] calculate (clip(abs(x) + qb))^2:\n");
        mmul_w_mm(TILE_NUM+0, TILE_NUM+3, TILE_NUM+3); /* (clip(abs(x) + qb))^2  -> acc0 Q32.0 */
        // add_norm_v3_cnt(8 * 8);
        msync_spm();
        // dmaload_spm((uint64_t)qc_tile, C_temp_sp_addr_start, cachelines);
        ...
        mlce32_spm(TILE_NUM+3, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* qc -> acc3 Q32.0 */
        // printf("[DEBUG] calculate q_poly: (clip(abs(x) + qb))^2 + qc:\n");
        madd_w_mm(TILE_NUM+0, TILE_NUM+0, TILE_NUM+3); /* (clip(abs(x) + qb))^2 + qc  -> acc0 Q32.0 */
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG] calculate erf:\n");
        mmul_w_mm(TILE_NUM+0, TILE_NUM+0, TILE_NUM+2); /* erf : ((clip(abs(x) + qb))^2 + qc) * sign x  -> acc0 Q32.0 */
        // add_norm_v3_cnt(8 * 8);
        msync_spm();
        ...
        mlce32_spm(TILE_NUM+2, C_temp_sp_addr_start, DIM_J * sizeof(acc_t), 0); /* q1 -> acc2 Q32.0 */
        madd_w_mm(TILE_NUM+0, TILE_NUM+0, TILE_NUM+2); /* erf + q1  -> acc0 Q32.0 */ 
        // add_norm_v3_cnt(8 * 8);
        // printf("[DEBUG] calculate IGELU:\n");
        mmul_w_mm(TILE_NUM+0, TILE_NUM+0, TILE_NUM+1); /* GELU(x) = x * (erf + q1)  -> acc0 Q32.0 */
        // add_norm_v3_cnt(8 * 8);
...
static void tiled_norm(size_t dim_I, size_t dim_J, const acc_t* in, acc_t* out,
  ...
  if (act == SOFTMAX) {
    ...
    acc_t qq1 = (acc_t)(q1 * intermediate_scale); // q1 Q32.0
    acc_t qb_neg_acc = (acc_t)(qb *(-1) * intermediate_scale); // -qb Q32.0
    acc_t qc_acc = (acc_t)(qc * intermediate_scale); // qc Q32.0
