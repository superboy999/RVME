#include "/cluster/home/geyh/RV/xuantie_gnu_toolchain/test/rvme/include/matmul_v2_2_2.h"
...
static alignas(CACHELINE_SIZE) elem_t Q_inRR[SEQ_LEN / 4][4*HIDDEN_DIM] = {[0 ... SEQ_LEN /4-1][0 ... 4*HIDDEN_DIM-1] = 1};
...
void rand_init_matmul() {
    ...
    for (int num = 0; num < NUM_HEADS; num++) {
        for (int i = 0; i < SEQ_LEN; i++) {
            for (int j = 0; j < SEQ_LEN; j++) {
                ...
                softmax_in[num][i][j] = (int32_t)((i + 10 * j + 100 * num)% 256 - 128); 
            }
        }
    }
    ...
    for (int i = 0; i < SEQ_LEN; i++) {
        for (int j = 0; j < EXPANSION_DIM; j++) {
            ...
            ffn_gelu_in[i][j] = (int32_t)((i + j)% 256 - 128); // -128 ~ 127
        }
    }
}
...
void reorder_Q_inR_to_256_per_head() {
    const int hd = HIDDEN_DIM / NUM_HEADS;
    const int block_rows = 256 / hd;
    const int new_rows = SEQ_LEN / block_rows;

    for (int head = 0; head < NUM_HEADS; head++) {
        int head_col_base_in  = head * hd;
        int head_col_base_out = head * 256;

        for (int r = 0; r < SEQ_LEN; r++) {
            int new_r = r / block_rows;
            int row_offset = r % block_rows;

            for (int c = 0; c < hd; c++) {
                int new_c = head_col_base_out + row_offset * hd + c;

                Q_inRR[new_r][new_c] =
                    Q_inR[r][head_col_base_in + c];
            }
        }
    }
}
...
)
{
    ...
    dump_matrix_int8(fp, "Q_inRR", Q_inRR, SEQ_LEN / 4, 4 * HIDDEN_DIM);
...
int32_t gelu_golden_q32_to_q16(acc_t x_q32, acc_scale_t scale, acc_scale_t bert_scale)
{
    /* 常数 */
    ...
    acc_t y_rounded_q16 = (acc_t)(y_rounded * (1 << 16));  // scale to Q16.16

    /* clip 到 int32 范围 */
    if (y_rounded_q16 > INT32_MAX) y_rounded_q16 = INT32_MAX;
    if (y_rounded_q16 < INT32_MIN) y_rounded_q16 = INT32_MIN;


    return (acc_t) y_rounded_q16;
}
...
void dump_gelu_results(
    ...
    /* ================= 输入矩阵 ================= */
    fprintf(fp, "===== GELU INPUT (Q32.0(Q8.0 in fact)) =====\n");
    for (int i = 0; i < SEQ_LEN; i++) {
        for (int j = 0; j < EXPANSION_DIM; j++) {
...
int main() {
    ...
    attention(HIDDEN_DIM, NUM_HEADS, SEQ_LEN,
        ...
        (const acc_t *)Wq_b, (const acc_t *)Wk_b, (const acc_t *)Wv_b,
        (acc_t *)Q_buf, (acc_t *)K_buf, (acc_t *)V_buf,
        (const elem_t *)Q_inRR, (const elem_t *)KT_in,
        (acc_t *)attn_buf, 
        (const acc_t *)softmax_in, (acc_t *)softmax_attn_buf,
        (const elem_t *)attn_in, (const elem_t *)V_in,
        (acc_t *)out_buf,
    ...
    dump_qkv_report(
        "/cluster/home/geyh/RV/xuantie_gnu_toolchain/test/rvme/tests/final_output/QKV_cp.out",
    ...
    // dump_attn_report(
    //     "/cluster/home/geyh/RV/xuantie_gnu_toolchain/test/rvme/tests/final_output/attn_cp.out",
    //     (const elem_t*)Q_in,
    //     (const elem_t*)KT_in,
    ...
    // dump_softmax_report(
    //     "/cluster/home/geyh/RV/xuantie_gnu_toolchain/test/rvme/tests/final_output/softmax_cp.out",
    //     softmax_in,
    //     softmax_attn_buf,
    //     softmax_attn_buf_g,
    //     NUM_HEADS,
    ...
    // // dump + compare
    // dump_resadd_report(
    //     "/cluster/home/geyh/RV/xuantie_gnu_toolchain/test/rvme/tests/final_output/resadd_cp.out",
    //     resadd_input0,
    //     resadd_input,
    //     resadd_out,
    //     resadd_out_g
    ...
    //     "/cluster/home/geyh/RV/xuantie_gnu_toolchain/test/rvme/tests/final_output/layernorm_cp.out",
