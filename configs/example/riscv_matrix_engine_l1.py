...
# GENERAL OPTIONS
ps.add_option('--cmd',              type="string", default="/cluster/home/guoym/gem5_matrix_engine/o3_v0.6/tests/cwq/test/matmul_v2_3",
...
ps.add_option('--mem_size',         type="string", default='8192MB',
...
ps.add_option('--connect_to_l1_m',  type="int", default=False,
...
system.cpu.matrix_interface = MatrixEngineInterface(
    matrix_engine = MatrixEngine(
        matrix_rename = MatrixRename(
            tilePRFs = options.tileReg_num,
            accPRFs = options.accReg_num,
...
def connectMatrixCache(cpu, matrix_engine, l2bus):
    ...
    cpu.matrix_interface.matrix_engine.matrix_spm.dmaDevice.dmaPort = system.spmbuffer.cpu_side
    # matrix_engine.matrix_mmu.matrix_mem_ports = l2bus.cpu_side_ports

    # if(connect_to_l1M):
    system.spmbuffer.mem_side = l2bus.cpu_side_ports
...
# # 插入 MemCheckerMonitor 在 l2cache 和 l2bus 之间：
# system.memchecker = MemChecker()
# system.mem_checker = MemCheckerMonitor(memchecker=system.memchecker)
# system.l2cache.cpu_side = system.mem_checker.mem_side_port
# system.mem_checker.cpu_side_port = system.l2bus.mem_side_ports
...
system.mem_ctrl.dram = DDR4_2400_8x8(device_size = options.mem_size)
